...
export class MacrodistritoService {
    constructor(
      ...
      async update(id: number, data: UpdateMacrodistritoDto): Promise<Macrodistrito> {
        const nuevoMacrodistrito = await this.macrodistritoRepository.findOne({
          where: { macro_id: id },
        });
        if (!nuevoMacrodistrito) throw new NotFoundException(`Macrodistrito ${id} no encontrada`);
